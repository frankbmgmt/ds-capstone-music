...
Trouble doing it elegantly...first do it manually for these 23!  I left out "chorus" because that's most likely from notating the change from the verses.
...
wrd_yeah = str_detect(text, 'yeah'),        
wrd_life = str_detect(text, 'life'),
wrd_gonna = str_detect(text, 'gpnna'),
wrd_heart = str_detect(text, 'heart'),
wrd_day = str_detect(text, 'day'),
wrd_night = str_detect(text, 'night'), 
wrd_feel = str_detect(text, 'feel'),
wrd_girl = str_detect(text, 'girl'), 
wrd_wprld = str_detect(text, 'world'),
wrd_home = str_detect(text, 'home'),
wrd_hey = str_detect(text, 'hey'), 
wrd_eyes = str_detect(text, 'eyes'), 
wrd_hear = str_detect(text, 'hear'), 
wrd_la = str_detect(text, 'la'),
wrd_mind = str_detect(text, 'mind'), 
wrd_wanna = str_detect(text, 'wanna'), 
wrd_leave = str_detect(text, 'leave'),
wrd_gotta = str_detect(text, 'gotta'),
wrd_tonight = str_detect(text, 'tonight'), 
)
...
for(i in 1:23) {
 ...
 key_word = train_dat_unnest [i,1]
  ...
  mutate(col_text = str_detect(text, key_word))
}

final_training_df

```
I can't figure out why final_training_df = NULL!!!!!

Let's try to find unique or close to unique words, in each Genre.
